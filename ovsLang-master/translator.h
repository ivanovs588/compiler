#ifndef Ovs_TRANSLATOR_H
#define Ovs_TRANSLATOR_H
...
public:
    ...
    Poliz *translate() {
        ...
        nowToken = &tokens[0];
        levels = {0};
        std::vector<std::string> reservedFunctions = {"print", "input", "int", "str", "bool", "size", "max", "min",
                                                      "double"};
        std::vector<std::string> reservedClasses = {"vector"};
        scopes.push_back({});
        bigScopes.push_back({});
...
//                                    nowToken->value);
    ...
    bool readSimpleStmt() {
        return readFlowStmt() || readExprStmt();
    }
    ...
    bool readFuncdef() {
        ...
        if (isDefines(nowToken->value))
            throw Exception("semantic error: redefinition " + nowToken->value,
                            ...
                            nowToken->numPos);
        if (nowToken->type != Token::NAME)
            throw Exception("expected NAME after \"function\"",
                            nowToken->numLine,
                            nowToken->numPos);
    ...
    bool readForStmt() {
        ...
        scopes.push_back({Scope::SIMPLE, ""});
        bigScopes.back().countCycle++;
        int firstBlock = nowPoliz->operations.size();
        if (!readExprStmt())
        ...
        if (!isOperator(","))
            throw Exception("expected ',' in for",
                            nowToken->numLine,
                            nowToken->numPos);
        getToken();
        int secondBlock = nowPoliz -> operations.size();
        ...
        // ~~~
            ...
            int jmpToSuite = nowPoliz->operations.size();
            addElement({0, Element::JMP});
        // ~~~
        ...
        if (!isOperator(","))
            throw Exception("expected ',' in for",
                            ...
                            nowToken->numPos);
        getToken();
        int thirdBlock = nowPoliz -> operations.size();
        if (!readExprStmt())
            throw Exception("invalid for condition",
                            nowToken->numLine,
                            nowToken->numPos);
        // ~~~
            addElement({0, Element::JMP, secondBlock});
        // ~~~
        ...
        if (!isBeginBlock("{"))
            throw Exception("expected '{'",
                            nowToken->numLine,
                            nowToken->numPos);
        nowPoliz->operations[jmpToSuite].posJump = nowPoliz->operations.size();
        if (!readSuite())
            throw Exception("invalid for suite",
                            nowToken->numLine,
                            nowToken->numPos);
        // ~~~
            addElement({0, Element::JMP, thirdBlock});
            nowPoliz->operations[badJmpPos].posJump = nowPoliz->operations.size();
        // ~~~
        popScope();
    ...
    bool readSuite() {
        ...
        while (readStmt()) {}
        if (nowToken->type != Token::END_BLOCK)
            throw Exception("expected '}' ",
                                  nowToken->numLine,
                                  nowToken->numPos);
        getToken();
