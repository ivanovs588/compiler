#ifndef Ovs_TOKENIZER_H
#define Ovs_TOKENIZER_H
...
private:
    ...
    const std::vector<std::string> keywords = {
            "if", "while", "for", "else", "elif", "break",
            "continue", "return", "function",
    ...
    void parseLine(int &pos, int &line, int &posInLine, std::vector<Token> &ans) {
        while (!isEnd(pos)) {
            if (input[pos] == '\n') {
                ...
                ans.push_back(Token(Token::END_BLOCK, line, posInLine, "}"));
                pos++;
                posInLine++;
            }
